...
def gumbel_stochasticrank(
    ...
    if sampled_rankings is None:
        (sampled_rankings, _, _, _, sampled_values) = pl.gumbel_sample_rankings(
            scores, n_samples, cutoff=cutoff + 1, return_gumbel=True
        )
    else:
    ...
    delta_pairwise = delta_weights[None, :, None] * (
        labels[None, None, :] - sampled_labels[:, :cutoff, None]
    )

    sampled_grad = pairwise_pdf * delta_pairwise
    sampled_mask = np.greater_equal(
        sampled_values[:, None, :], ranking_values[:, :cutoff, None]
    )
    ...
    pairwise_pdf = np.exp(-pairwise_diff - np.exp(-pairwise_diff))
    delta_moved = delta_weights[None, :, None] * (
        sampled_labels[:, None, :cutoff] - sampled_labels[:, 1:, None]
    )
    ...
    np.add.at(
        sampled_grad,
        (np.arange(n_samples)[:, None], sampled_rankings[:, :cutoff]),
        grad_cor,
    )
...
def normal_pdf(x):
    return np.exp(-(x**2.0) / (2.0)) / (2.0 * np.pi * 1.0) ** 0.5


def normal_stochasticrank(
    rank_weights,
    labels,
    scores,
    n_samples=None,
    ...
    assert n_samples is not None or sampled_rankings is not None
    if sampled_rankings is None:
        (sampled_rankings, _, sampled_values) = pl.normal_sample_rankings(
            scores, n_samples, cutoff=cutoff + 1, return_gumbel=True
        )
    else:
        n_samples = sampled_rankings.shape[0]
