...
import numpy as np
from ..utils import plackettluce as pl
from ..utils.sample_group_assignment import GDL23
from ..utils import ranking as rnk
...
def process_rankings(
    sampled_group_assignments, group_ids, scores, labels, rank_weights, group_n_samples
):
    n_docs = len(scores)
    ...
    for j in range(n_groups):
        group_scores.append(np.zeros(n_docs))
        group_rel_labels.append(np.zeros(n_docs))
        group_scores[j][group_ids[j]] = scores[group_ids[j]]
        group_rel_labels[j][group_ids[j]] = labels[group_ids[j]]

    chunk_result = np.zeros(n_docs, dtype=np.float64)
    for x in sampled_group_assignments:
        for j in range(n_groups):
            group_result = np.zeros(n_docs, dtype=np.float64)
            ...
            if (
                ...
                continue
            group_cutoff = min(len(group_ranks), len(group_ids[j]))
            if group_cutoff == 0:
                continue
            ...
            chunk_result += group_result
    return chunk_result


# def process_rankings(args):
#     sampled_group_assignments = args[0]
#     group_ids = args[1]
#     scores = args[2]
#     labels = args[3]
#     rank_weights = args[4]
...
def Group_Fair_PL(
    ...
    n_groups = len(fairness_constraints[0])
    n_docs = labels.shape[0]

    cutoff = min(rank_weights.shape[0], n_docs)

    # extract the gropu-wise ids
    group_ids = []
    for j in range(n_groups):
        ids = np.where(group_membership == j)[0]
    ...
    # gradients = np.sum(results, axis=0)
...
#     # before = time.time()
#     num_processes = 12  # use 12
#     pool = Pool(num_processes)  # Create a multiprocessing Pool
#     if len(sampled_group_assignments) >= num_processes:
#         chunk_size = len(sampled_group_assignments) // num_processes
#     else:
#         chunk_size = len(sampled_group_assignments)

#     data_chunks = [
#         [
#             sampled_group_assignments[i : i + chunk_size],
#             group_ids,
#             scores,
#             labels,
#             rank_weights,
#         ]
#         for i in range(0, len(sampled_group_assignments), chunk_size)
#     ]

#     results = pool.map(process_rankings, data_chunks)

#     gradients = np.sum(results, axis=0)
#     # print("Time with multiprocessing: ", time.time()-before)
