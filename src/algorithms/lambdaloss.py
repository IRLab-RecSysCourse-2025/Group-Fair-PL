...
def lambdaloss(rank_weights, labels, scores):
    n_docs = scores.shape[0]
    cutoff = min(rank_weights.shape[0], n_docs)

    (greater_i, lesser_i) = np.where(np.greater(labels[:, None], labels[None, :]))
    ...
    delta_rank = np.abs(inv_ranking[greater_i] - inv_ranking[lesser_i])
    if n_docs > cutoff:
    ...
    delta_weight = safe_rank_weights[delta_rank - 1] - safe_rank_weights[delta_rank]
    pair_weight = delta_weight * (labels[greater_i] - labels[lesser_i])

    exp_score_diff = np.exp(scores[lesser_i] - scores[greater_i])

    pair_deriv = pair_weight * exp_score_diff / ((exp_score_diff + 1.0) * np.log(2.0))
