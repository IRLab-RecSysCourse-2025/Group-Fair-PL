...
def get_dataset_from_json_info(
    ...
    return DataSet(
        name=dataset_name,
        data_paths=set_info["fold_paths"],
        num_rel_labels=set_info["num_relevance_labels"],
        num_features=num_feat,
        read_from_pickle=read_from_pickle,
        num_nonzero_feat=set_info["num_nonzero_feat"],
...
class DataSet(object):
    ...
    def get_data_folds(self):
        if "German" in self.name:
            return [
                GermanDataFold(dataset=self, fold_num=i, data_path=path)
                for i, path in enumerate(self.data_paths)
            ]
        elif "HMDA" in self.name:
            return [
                pcaHMDADataFold(dataset=self, fold_num=i, data_path=path)
                for i, path in enumerate(self.data_paths)
            ]
        elif "MOVLENS" in self.name:
            return [
                MOVLENSDataFold(dataset=self, fold_num=i, data_path=path)
                for i, path in enumerate(self.data_paths)
            ]
        else:
...
class DataFoldSplit(object):
    ...
    def query_gender(self, query_index):
        s_i = self.doclist_ranges[query_index]
        e_i = self.doclist_ranges[query_index + 1]
        return self.gender_vector[s_i:e_i, :]
...
class GermanDataFold(object):
    ...
    def _read_file(self, path, purge):
        """
        Read letor file.
        """
        queries = []
        ...
        for line in tqdm(open(path, "r")):
            ...
            qid = info[1].split(":")[1]
            label = float(info[0].split(":")[1])
            gender = int(float(info[5].split(":")[1]))
            feat_pairs = info[2:]
            ...
            if current_qid is None:
                ...
                stacked_documents = np.stack(cur_docs, axis=0)
                # if self.feature_normalization:
                #     stacked_documents -= np.amin(stacked_documents, axis=0)[
                #         None, :
                #     ]
                #     safe_max = np.amax(stacked_documents, axis=0)
                #     safe_max[safe_max == 0] = 1.0
                ...
                np_genders = np.array(cur_genders, dtype=np.int64)
                if not purge or np.any(np.greater(np_labels, 0)):
                    queries.append(
                        {
                            "qid": current_qid,
            ...
            cur_docs.append(doc_feat)
            cur_labels.append(label)
            cur_genders.append(gender)

        all_docs = np.concatenate([x["documents"] for x in queries], axis=0)
        all_n_docs = np.array([x["n_docs"] for x in queries], dtype=np.int64)
        all_labels = np.concatenate([x["labels"] for x in queries], axis=0)
        all_genders = np.concatenate([x["genders"] for x in queries], axis=0)

        query_ranges = _add_zero_to_vector(np.cumsum(all_n_docs))

        return query_ranges, all_docs, all_labels, all_genders
    ...
    def read_data(self):
        ...
        if not data_read:
            (
                ...
                train_label_vector,
                train_gender_vector,
            ) = self._read_file(train_raw_path, False)
            (
                valid_doclist_ranges,
                valid_feature_matrix,
                valid_label_vector,
                valid_gender_vector,
            ) = self._read_file(valid_raw_path, False)
            (
                test_doclist_ranges,
                test_feature_matrix,
                test_label_vector,
                test_gender_vector,
            ) = self._read_file(test_raw_path, self.purge_test_set)
            ...
            #         valid_doclist_ranges, valid_feature_matrix
            #     )
            #     self._normalize_feat(test_doclist_ranges, test_feature_matrix)

            #     list_map = [
            #         x[0]
            #         for x in sorted(feature_map.items(), key=lambda x: x[1])
            #     ]
            #     filtered_list_map = [
            #         x for i, x in enumerate(list_map) if non_zero_feat[i]
            #     ]
            ...
            #         test_doclist_ranges=test_doclist_ranges,
            #         test_label_vector=test_label_vector,
            #         test_gender_vector=test_gender_vector
            #     )

        # n_feat = len(feature_map)
        # assert (
        #     n_feat == self.num_features
        # ), "%d features found but %d expected" % (n_feat, self.num_features)
...
class HMDADataFold(object):
    ...
    def _read_file(self, path, purge):
        ...
        for line in tqdm(open(path, "r")):
            ...
            qid = info[1].split(":")[1]
            label = float(info[0].split(":")[1])
            gender = int(float(info[2].split(":")[1]))
            feat_pairs = info[2:]  # make it [2:-1] if you want to look at color-blind
    ...
    def read_data(self):
        ...
        train_raw_path = self.data_path + "train.txt"
        valid_raw_path = self.data_path + "vali.txt"  # there was no validation set
        test_raw_path = self.data_path + "test.txt"
...
class MOVLENSDataFold(object):
    ...
    def _read_file(self, path, purge):
        ...
        for line in tqdm(open(path, "r")):
            ...
            label = float(info[0].split(":")[1])
            qid = info[1].split(":")[1]
            gender = int(float(info[52].split(":")[1]))
            feat_pairs = info[2:]
